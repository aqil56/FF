...
# from ..feastfreedom.settings import aws_access_key, aws_secret_key, email
    ...
    def new_customer(request):
        if request.method == 'POST':
            ...
            if f.is_valid():
                ...
                # response = client.verify_email_identity(EmailAddress=f.email)
...
def OrderCreate(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            kitchen_id = ''
            for item in cart:
                OrderItem.objects.create(order=order,food=item['item'],price=item['price'],quantity=item['quantity'])
                kitchen_id = item['item'].kitchen_id
            cart.clear()
            request.session['order_id'] = order.id
            return HttpResponseRedirect(reverse_lazy('kitchen:order_succes',args=(kitchen_id,)))
    else:
        form = OrderCreateForm()
    return render(request, 'order/order_create.html', {'cart': cart,
                                                        'form': form})

def OrderSuccess(request, kitchen_id):
    kitchen = get_object_or_404(Kitchen, id=kitchen_id)
    provider_email = kitchen.customer.email
    customer = get_object_or_404(Customer, username=request.user)
    customer_email = customer.email
    # check if the email is verified
    # response = client.list_verified_email_addresses()
    # if customer_email in response['VerifiedEmailAddresses'] and provider_email in response['VerifiedEmailAddresses']:
    # #     send email
    #     client.send_email(
    #         Destination={
    #             'ToAddresses': [customer_email, provider_email],
    #         },
    #         Message={
    #             'Body': {
    #                 'Text': {
    ...
    #             'Subject': {
    #                 'Charset': 'UTF-8',
    #                 'Data': 'FeastFreedom Order Confirmation',
    #             },
    #         },
    #         Source=email,
    #     )
